...
OVIS is published on GitHub as the public open-source distribution of the project. Development happens in an internal GitLab repository, and validated public changes are synchronized here after sanitization.

See [CHANGELOG.md](CHANGELOG.md) for user-facing changes and public release notes.
...
The public GitHub repository is the user-facing open-source distribution of OVIS. It is synchronized from the canonical GitLab `main` branch and intentionally omits internal operator assets that are not required for the default demo-mode setup:

- `Backend/Data-Import/onkostar/sql/*.sql`
- `Backend/Apollo/Certificates/lmu.pem`
- top-level `scripts/`
- top-level `docs/`
- top-level `ccp/`
- generated or placeholder CCP omock outputs under `Backend/Data-Import/ccp/`

The default public setup uses `OVIS_IMPORT_MODE=demo`, which is already set in `.env.example`. ONKOSTAR mode requires private SQL statements and database access that are not included in the public mirror. Image publishing helpers are maintained in the canonical source repository and are not part of the public checkout.

### Image-only runtime

If prebuilt images are available for your deployment, you can use the image-only compose file:
...
*   **`OVIS_NO_PROXY`**: Comma-separated hosts/domains that should bypass the proxy (example: `localhost,127.0.0.1,::1,keycloak,postgres,nginx,express-auth,ovis-frontend,ovis-frontend-dev,ovis-backend-apollo,ovis-backend-database-mongodb,ovis-backend-mongodb-data-preprocessing,ovis-backend-data-import,mongodb-restore,mongodb-backup,.example.org,.example.net,.svc,.cluster.local`)

These values are consumed by build/runtime-sensitive services (frontend, apollo, express-auth, data-import, preprocessing, keycloak). No proxy is used unless you explicitly set them.
...
The preprocessing service (`ovis-backend-mongodb-data-preprocessing`) is built as a self-contained image with all dependencies baked in at build time. This avoids race conditions from shared `node_modules` volumes.

For a fresh public demo-mode restart, use:

```bash
OVIS_IMPORT_MODE=demo docker compose down -v && OVIS_IMPORT_MODE=demo docker compose up --build -d
```

ONKOSTAR fresh-start smoke tests require private SQL statements and database access that are not included in the public GitHub mirror.

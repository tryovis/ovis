...
const computedFields = aggre[0].$set;

const getAggre = (kpType, stat, group, type) => {
	const setFields = {};
	[kpType, stat].forEach((field) => {
		if (computedFields[field]) {
			setFields[field] = computedFields[field];
		}
	});
	...
	const project = {
		$project: {
			...
			dateDiff: `$${kpType}`,
			status: `$${stat}`,
			groupe: group ? { $ifNull: [`$${group}`, '0'] } : '0',
			tumorID: 1
		}
	};

	return [...(Object.keys(setFields).length ? [{ $set: setFields }] : []), match, sort, project];
};
...
module.exports = {
	Query: {
		getSurvivalKaplanMeierChart: async (
			...
			pipeline.push(...getAggre(date, stat, group, type));

<script lang="ts">
	...
	type InputType = {
		category: string[];
		groups: [{ count: number[]; gender: string; label: string; description: string }];
	};
	...
	function mergeCountsToLabels(
		groups: { count: number[]; gender: string; label: string; description: string }[]
	) {
		const map: {
			[key: string]: { count: number[]; gender: string; label: string; description: string };
		} = {};
		...
		groups.forEach((element) => {
			if (!map[element.label]) {
				...
				map[element.label].count = map[element.label].count.map(
					(value, index) => value + element.count[index]
				);
			}
		});
	...
	onMount(async () => {
		...
		abscissaKey =
			typeof selectedAbscissa.label === 'string'
	...
	async function createBarChart() {
		...
		const requestedSelectedFeature = selectedFeature;
		const requestedSelectedAbscissa = selectedAbscissa;
		const requestedAbscissaKey = abscissaKey;
		const requestedShowTop5 = showTop5;
		const requestedAspectRatio = aspectRatio;
		...
		const result = await getDiagnosisBarChart(
			{
				genderWise: requestedSelectedGender,
				group: requestedSelectedFeature.value,
				abscissa: requestedSelectedAbscissa.value
			},
			requestedFilter
		);
		...
		barChartData2 = [];
		inputArray = result;
		if (requestedSelectedAbscissa.value === 'none') {
		...
		for (let i = 0; i < inputArray.groups.length; i++) {
			inputArray.category.forEach((category, index) => {
				...
				tableData.push({
					[requestedAbscissaKey]: category,
					gender: gender,
					[requestedSelectedFeature.value + 'Text']: description,
		...
		if (requestedShowTop5) {
			...
			let topCategoriesInputArray = inputArray.groups.filter((element) =>
				top5HashMap.some((item) => item[0] === element.label)
			);
			let restInputArray = inputArray.groups.filter(
				(element) => !top5HashMap.some((item) => item[0] === element.label)
			);
			...
			restInputArray.forEach((element) => {
				...
				if (!found) {
					countsObjectArray.push({
						count: Array(inputArray.category.length).fill(0),
						label: 'Sonstige',
						gender: element.gender
					});
		...
		uniqueStackLabels = new Set(
			[...uniqueStackLabels].map((value) => (value === null ? 'x' : value))
		);
		x2axis = [...uniqueStackLabels].join(' | ');

		const resetSliderRange = !mounted || selectedAbscissaChanged;
		mounted = true;
		await tick();
		if (loadToken !== chartLoadToken) return;
		if (!barChart) return;
			...
			options: {
				...
				scales: {
					...
					x: {
						...
						display: requestedSelectedGender ? true : false
					},
					x2: {
						border: {
							display: false
				...
				onClick: (e, elements) => {
					if (elements.length > 0) {
						...
						if (requestedSelectedGender) {
							if (gender !== 'x') {
								console.log(
									`Bar clicked: ${label} in category ${requestedSelectedAbscissa.label}: ${category} with gender ${gender}`
								);
								let queryItem2 = {
							...
							console.log(
								`Bar clicked: ${label} in category ${requestedSelectedAbscissa.label}: ${category}`
							);
						}

						if (requestedAbscissaKey) {
							console.log('ENTERED ABSCICSSA KEY', requestedAbscissaKey);
							let queryItem3 = {
								id: 'Random generierte UUID', //uuidv4(),
								key: requestedAbscissaKey, //theoretisch metastasis => Im Katalog hinterlegt
								name: 'childCategorie.name', //Im Katalog hinterlegt
								type: 'EQUALS',
								system: 'diagnosis',
								values: [
	...
	function setSlider(resetSliderRange = !mounted || selectedAbscissaChanged) {
		destroySlider();
	...
	<div class="dropdown-container">
		<div class="hdropdown">
			...
			<div class="menu">
				<ul>
					<li>
						{selectedAbscissa.label}<!-- prettier-ignore --><select class="carret-decoration"></select>
						<ul>
							<li class="link">
								<button
									on:click={() => (selectedAbscissa = { value: 'none', label: $t('totalCohort') })}
									>{$t('totalCohort')}</button
								>
							</li>
							<li class="link">
								<button
									on:click={() =>
										(selectedAbscissa = { value: 'ICD_ICD10_3', label: $t('entityICD10_3digit') })}
									>{$t('entityICD10_3digit')}</button
								>
							</li>
							<li>
								{$t('time')}
								<ul>
									<li class="link">
										<button
											on:click={() => (selectedAbscissa = { value: 'weeks', label: $t('weeks') })}
											>{$t('weeks')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() => (selectedAbscissa = { value: 'months', label: $t('months') })}
											>{$t('months')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedAbscissa = { value: 'quarters', label: $t('quarters') })}
											>{$t('quarters')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() => (selectedAbscissa = { value: 'years', label: $t('years') })}
											>{$t('years')}</button
										>
									</li>
								</ul>
		...
		<div class="hdropdown">
			...
			<div class="menu">
				<ul>
					<li>
						{selectedFeature.label}<!-- prettier-ignore --><select class="carret-decoration"></select>
						<ul>
							<li class="link">
								<button
									on:click={() =>
										(selectedFeature = { value: 'noGroup', label: $t('noSelection') })}
									>{$t('noSelection')}</button
								>
							</li>
							<li>
								{$t('tumorFeature')}
								<ul>
										...
										<ul>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = {
															value: 'ICD_ICD10Group',
															label: $t('ICD10_grouped')
														})}>{$t('ICD10_grouped')}</button
												>
											</li>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = { value: 'ICD_ICD10_3', label: $t('ICD10_3digit') })}
													>{$t('ICD10_3digit')}</button
												>
											</li>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = { value: 'ICD_ICD10', label: $t('ICD10_detailed') })}
													>{$t('ICD10_detailed')}</button
												>
											</li>
										</ul>
									</li>
											...
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = {
															value: 'grading_first',
															label: $t('gradingFirst')
														})}>{$t('gradingFirst')}</button
												>
											</li>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = { value: 'grading_last', label: $t('gradingLast') })}
													>{$t('gradingLast')}</button
												>
											</li>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = {
															value: 'grading_lowest',
															label: $t('gradingLowest')
														})}>{$t('gradingLowest')}</button
												>
											</li>
											<li class="link">
												<button
													on:click={() =>
														(selectedFeature = {
															value: 'grading_highest',
															label: $t('gradingHighest')
														})}>{$t('gradingHighest')}</button
												>
											</li>
										</ul>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = { value: 'metastasis', label: $t('metastases') })}
											>{$t('metastases')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'ICDO_localizationCode',
													label: $t('ICDO_localization')
												})}>{$t('ICDO_localization')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'ICDO_histologyCode',
													label: $t('ICDO_histology')
												})}>{$t('ICDO_histology')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() => (selectedFeature = { value: 'side', label: $t('side') })}
											>{$t('side')}</button
										>
									</li>
									...
									{#if !isCCP}
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'diagnosisAssurance',
														label: $t('diagnosisConfirmation')
													})}>{$t('diagnosisConfirmation')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														...
														label: $t('diagnosisReason')
													})}>{$t('diagnosisReason')}</button
											>
										</li>
									{/if}
								</ul>
								...
								<ul>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'previousTherapy_surgery',
													label: $t('receivedSurgery')
												})}>{$t('receivedSurgery')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'previousTherapy_radiation',
													label: $t('receivedRadiation')
												})}>{$t('receivedRadiation')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'previousTherapy_systemic',
													label: $t('receivedSystemicTherapy')
												})}>{$t('receivedSystemicTherapy')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'previousDiagnostic_radiology',
													label: $t('receivedRadiologicalDiagnostics')
												})}>{$t('receivedRadiologicalDiagnostics')}</button
										>
									</li>
								...
								<ul>
									{#if !isCCP}
										<li>
											...
											<ul>
												<li class="link">
													<button
														on:click={() =>
															(selectedFeature = { value: 'ECOG_first', label: $t('ECOGFirst') })}
														>{$t('ECOGFirst')}</button
													>
												</li>
												<li class="link">
													<button
														on:click={() =>
															(selectedFeature = { value: 'ECOG_last', label: $t('ECOGLast') })}
														>{$t('ECOGLast')}</button
													>
												</li>
												<li class="link">
													<button
														on:click={() =>
															(selectedFeature = { value: 'ECOG_lowest', label: $t('ECOGLowest') })}
														>{$t('ECOGLowest')}</button
													>
												</li>
												<li class="link">
										...
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = { value: 'distress', label: $t('distressPresent') })}
												>{$t('distressPresent')}</button
											>
										</li>
									...
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = { value: 'VitalState', label: $t('vitalStatus') })}
											>{$t('vitalStatus')}</button
										>
									</li>
									<li class="link">
										<button
											on:click={() =>
												(selectedFeature = {
													value: 'ageAtDiagnosisGroup',
													label: $t('ageGroupAtDiagnosis')
												})}>{$t('ageGroupAtDiagnosis')}</button
										>
									</li>
								</ul>
							</li>
									...
									<ul>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousTumorboard_any',
														label: $t('tumorboardAny')
													})}>{$t('tumorboardAny')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousTumorboard_prae',
														label: $t('tumorboardPrae')
													})}>{$t('tumorboardPrae')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousTumorboard_post',
														label: $t('tumorboardPost')
													})}>{$t('tumorboardPost')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousTumorboard_mtb',
														label: $t('tumorboardMTB')
													})}>{$t('tumorboardMTB')}</button
											>
										</li>
									</ul>
								</li>
									...
									<ul>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousConsultation_social',
														label: $t('social')
													})}>{$t('social')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousConsultation_psycho',
														label: $t('psychooncological')
													})}>{$t('psychooncological')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousConsultation_nutrition',
														label: $t('nutrition')
													})}>{$t('nutrition')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = {
														value: 'previousConsultation_genetic',
														label: $t('genetic')
													})}>{$t('genetic')}</button
											>
										</li>
								...
								<li>
									...
									<ul>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = { value: 'primaryCase', label: $t('primaryCase') })}
												>{$t('primaryCase')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = { value: 'centerCase', label: $t('centerCase') })}
												>{$t('centerCase')}</button
											>
										</li>
										...
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = { value: 'recurrence', label: $t('recurrence') })}
												>{$t('recurrence')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													...
													})}>{$t('organizationalUnit')}</button
											>
										</li>
										<li class="link">
											<button
												on:click={() =>
													(selectedFeature = { value: 'internal', label: $t('internalExternal') })}
												>{$t('internalExternal')}</button
											>
										</li>

...
**NEVER commit private key files to version control (common extensions: .key, .pem, .p12, .pfx)!**
...
> Note: `.pem`, `.crt`, and `.key` are mostly filename conventions.
> What matters is content/format (PEM-encoded certificate + matching private key) and correct paths in `.env`.
...
# If HTTPS is enabled and these files are missing, nginx generates a self-signed
# development certificate automatically inside the mounted certificate directory.
```

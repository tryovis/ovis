...
export const getTumorboardTable = (
	...
	graphqlFetch(dataUrl, {
		...
		.then((result) => {
			result.data.getTumorBoard.forEach((element) => {
				if (element.tumorBoardOccurrenceDate)
					element.tumorBoardOccurrenceDate = new Date(
						element.tumorBoardOccurrenceDate
					).toLocaleDateString('de-DE', localeOptions);
			});
